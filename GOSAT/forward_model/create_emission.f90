...
REAL(8):: P(nstate),T(nstate),dx(nstate),xCO2(nstate), xH2O(nstate), xCO(nstate)
REAL(8):: angel(2)
...
INTEGER:: wavesize
INTEGER:: absco_size
REAL(8), ALLOCATABLE:: dab_dx_in(:)	
REAL(4), ALLOCATABLE:: absco(:,:),absco_in(:)
...
ALLOCATE(absco(absco_size,nstate),absco_in(absco_size))
...
CALL interp(P(i),T(i),xCO2(i), 0.d0, 0.d0, absco_wave, absco_size, absco_in(:), dab_dx_in(:))
absco(:,i)=absco_in(:)
END DO
call cpu_time(t4)
WRITE(*,*) 'Time=',t4-t3
...
DO i = 1,absco_size
    ...
    opticalThickness(i)=1000d0*100d0*(dx(1)-dx(2))*(absco(i,1) + Rayleigh(i,1))  !(absco(i,1) + Rayleigh(i,1))
    DO j=2,nstate-1
        ...
        opticalThickness(i)=opticalThickness(i)+1000d0*100d0*(dx(j)-dx(j+1))*(absco(i,j) + Rayleigh(i,j))  !(absco(i,j) + Rayleigh(i,j))
    END DO
Rayleigh(i,nstate) =1.d0/ (Ns* P(nstate)*1000/Ps *Ts/T(nstate)) *24.d0* pi**3 *(n(i)**2-1.d0)**2 /&
            &(1d4/absco_wave(i))**4/(n(i)**2+2.d0)**2 * (6.d0+3.d0*rn(i))/(6.d0-7.d0*rn(i))
    opticalThickness(i)=opticalThickness(i)+1000d0*100d0*dx(nstate)*(absco(i,nstate)+Rayleigh(i,nstate))
END DO
